...
class TubeTest {
    ...
    void testFindIntsersections() {
        ...
        assertNull(tube1.findIntsersections(ray),"Must not be intersections");

        // TC02: Ray's crosses the tube (2 points)
        ray = new Ray(new Point(0, 0, 0), new Vector(2, 1, 1));
        List<Point> result = tube2.findIntsersections(ray);
        ...
        result = tube2.findIntsersections(ray);
        assertNotNull( result,"must be intersections");
        ...
        assertNull(tube2.findIntsersections(ray), "must not be intersections" );
